...
void ist8310_RST_H(void)//复位IO 置高
{
...
uint8_t ist8310_init(void)
{
    ...
    ist8310_RST_H();
    ist8310_delay_ms(sleepTime);//复位

    res = ist8310_IIC_read_single_reg(IST8310_WHO_AM_I);//读取磁力计ID
    if (res != IST8310_WHO_AM_I_VALUE)
    {
        return IST8310_NO_SENSOR;//没有磁力计
    }
    ist8310_delay_ms(wait_time);
    //set mpu6500 sonsor config and check
    for (writeNum = 0; writeNum < IST8310_WRITE_REG_NUM; writeNum++)//设置传感器参数
    {
        ...
        {
            return ist8310_write_reg_data_error[writeNum][2];//返回错误
        }
    }
...
void ist8310_read_over(uint8_t *status_buf, ist8310_real_data_t *ist8310_real_data)
{
    ...
    {
        //状态寄存器地址为0X02，回传一个字节（8位），位于第3位（7：2）
        //1：数据溢出（旧数据未读取，新数据已经产生）
        //0：数据准备状态（新数据产生）
...
void ist8310_read_mag(fp32 mag[3])
{
    uint8_t buf[6];
    int16_t temp_ist8310_data = 0;
    ist8310_IIC_read_muli_reg(0x03, buf, 6);//输出数据寄存器 0X03
