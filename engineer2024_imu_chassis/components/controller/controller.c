 ...
 * @param[in]      略
 *什么都不用   NONE = 0X00,                        //0000 0000
 *积分输出上限 ，这个要开   Integral_Limit = 0x01,              //0000 0001   1
 *微分先行  Derivative_On_Measurement = 0x02,   //0000 0010
 *梯形积分 Trapezoid_Intergral = 0x04,         //0000 0100
 *Proportional_On_Measurement = 0x08, //0000 1000
 *输出滤波  OutputFilter = 0x10,                //0001 0000   1
 *变速积分  ChangingIntegrationRate = 0x20,     //0010 0000
 *微分滤波器  DerivativeFilter = 0x40,            //0100 0000
 *故障处理  ErrorHandle = 0x80,                 //1000 0000
 *需要开哪个就把对应的位置设为1就行，然后把对应的十六进制值0xoo输到最后一个参数里。
 * @retval         返回空      null
 */
void PID_Init(
    PID_t *pid, //声明的pid结构体
    float max_out,//pid的最大输出
    float intergral_limit,//积分项输出上限
    float deadband,//pid死区，可以参考PID进阶里的死区限制，不用的话设为0.0f

    float kp,
    float Ki,
    float Kd,

    float A,//变速积分参数，不用的话设为0.0f
    float B,//变速积分参数，不用的话设为0.0f

    float output_lpf_rc,//输出一阶滤波对应rc值，计算方法为RC = 1/(2*PI*截止频率Fc）
    float derivative_lpf_rc,//微分一阶滤波对应rc值，计算方法为RC = 1/(2*PI*截止频率Fc）

    uint16_t ols_order,//最小二乘求微分阶数，英雄目前给4

    uint8_t improve)//提高项设置参数
{
...
float PID_Angle_Calculate(PID_t *pid, float measure, float ref)
{
    if (pid->Improve & ErrorHandle)
        f_PID_ErrorHandle(pid);
...
void PID_Clear(PID_t *pid)
{
    ...
    pid->Err = 0;
    pid->Output =pid->Dout = pid->Iout = pid->Pout = 0.0f;        
    pid->Measure = pid->Measure = 0.0f;
}
