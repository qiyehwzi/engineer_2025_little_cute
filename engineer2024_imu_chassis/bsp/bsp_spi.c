...
{
    SET_BIT(hspi1.Instance->CR2, SPI_CR2_TXDMAEN);//发送DMA使能
    SET_BIT(hspi1.Instance->CR2, SPI_CR2_RXDMAEN);//接收DMA使能

    __HAL_SPI_ENABLE(&hspi1);//打开SPI
    ...
    __HAL_DMA_CLEAR_FLAG(&hdma_spi1_rx, DMA_LISR_TCIF2);//清除标志位
...
{
    __HAL_DMA_DISABLE(&hdma_spi1_rx);
    __HAL_DMA_DISABLE(&hdma_spi1_tx);//关闭DMA
