    ...
    HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &rx_header, rx_data);
		...
		{	
			if(rx_header.StdId == MST_1_ID)
			{
					...
					if(DM_motor_4310[0].motor_position - DM_motor_4310[0].last_position < -PI/2)
					{
						armend.motor_DM_data[0].round_cnt ++;
					}
					else if((DM_motor_4310[0].motor_position - DM_motor_4310[0].last_position > PI/2))
					{
						armend.motor_DM_data[0].round_cnt --;
					}
			}
			else if(rx_header.StdId == MST_2_ID)
			{
					...
					if(DM_motor_4310[1].motor_position - DM_motor_4310[1].last_position < -PI/2)
					{
						armend.motor_DM_data[1].round_cnt ++;
					}
					else if((DM_motor_4310[1].motor_position - DM_motor_4310[1].last_position > PI/2))
					{
						armend.motor_DM_data[1].round_cnt --;
					}
			}
			else if(rx_header.StdId == MST_3_ID)
			{
					...
					if(DM_motor_4310[2].motor_position - DM_motor_4310[2].last_position < -PI/2)
					{
						armend.motor_DM_data[2].round_cnt ++;
					}
					else if((DM_motor_4310[2].motor_position - DM_motor_4310[2].last_position > PI/2))
					{
						armend.motor_DM_data[2].round_cnt --;
					}			
			}
...
void get_DM_motor_1_measure(motor_DM_motor_t *DM_motor, uint8_t data[8])
{
	...
	DM_motor->last_position = DM_motor->motor_position;
